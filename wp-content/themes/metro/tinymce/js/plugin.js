	...
	tinymce.create('tinymce.plugins.om_shortcode_plugin', {
		...
		addPopup: function ( b, title, id ) {
			b.add({
				...
				onclick: function () {
					...
					tb_show("Insert Shortcode: "+title, OM_TEMPLATE_DIR_URI+'/tinymce/popup.php?shortcode=' + id + '&width=590&height='+(height-140) );
				}
			});
		},
		addInsert: function ( b, title, code_start, code_end) {
			b.add({
				title: title,
				onclick: function () {
					//tinyMCE.activeEditor.execCommand( "mceInsertContent", false, code_start+code_end );
